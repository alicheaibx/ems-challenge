...
const TimesheetsPage: React.FC = () => {
	...
	// Initialize the events service and calendar app
	...
	// Handle opening/closing the dialogs
	...
	const handleUpdateTimesheet = async (timesheet: {
		...
		try {
			const response = await fetch(
				`http://localhost:3000/timesheets/${timesheet.id}`,
				{
					method: "PUT",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify(timesheet),
	...
	useEffect(() => {
		if (calendarApp && calendarApp.events) {
			...
			// Log all events after adding
			console.log("All events after adding:", calendarApp.events.getAll());
		}
		...
		<Container>
			...
			<FormControl fullWidth margin="normal">
				...
				>
					...
					{Array.from(
						new Set(timesheets.map((timesheet) => timesheet.employee_id))
					).map((employeeId) => (
						<MenuItem key={employeeId} value={employeeId}>
							{timesheets.find(
								(timesheet) => timesheet.employee_id === employeeId
